...
if os.path.exists(dist):
    # Mount asset folders generated by Vite
    if os.path.exists(os.path.join(dist, "assets")):
        app.mount("/assets", StaticFiles(directory=os.path.join(dist, "assets")), name="assets")

    # Catch-all route for frontend views
    @app.get("/{full_path:path}")
    async def serve_frontend(full_path: str):
        # Prevent catching API routes if any slip through
        if full_path.startswith("api/") or full_path in ["todos", "protected", "docs", "openapi.json", "redoc"]:
